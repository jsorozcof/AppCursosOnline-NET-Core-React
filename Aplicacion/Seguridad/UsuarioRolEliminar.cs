    ...
    public class UsuarioRolEliminar
    {
        ...
        public class Manejador : IRequestHandler<Ejecuta>
        {
            ...
            public async Task<Unit> Handle(Ejecuta request, CancellationToken cancellationToken)
            {
                ...
                if(role==null){
                    throw new ManejadorExepcion(HttpStatusCode.NotFound, new {mensaje = "No se encontro el rol"});
                }

                var usuarioIden = await _userManager.FindByNameAsync(request.Username);
                if(usuarioIden==null){
                    throw new ManejadorExepcion(HttpStatusCode.NotFound, new {mensaje = "No se encontro el usuario"});
                }
